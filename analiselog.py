...
def autores_populares():
    ...
    c = pg.cursor()
    c.execute("""
                SELECT authors.name, count(log.path) AS total
                FROM authors
                JOIN articles 
...
def porcentagem():
    ...
    c.execute("""
                CREATE VIEW erro AS 
                SELECT 
                    to_char(time, 'Mon DD, YYYY') AS data,
    ...
    c.execute("""
                SELECT erro.data, 
                    ROUND(100.0 * erro.totalerro / nerro.totalnerro, 2)
                FROM erro 
                JOIN nerro 
                    ON nerro.data = erro.data
